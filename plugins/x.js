...
const UNQ = "wrong command dont type words after command (Powerful bug)"
const DDO = "```HIJACKED🤡```"
const ONO = ".tagall  ```Hello I Love you all🤡```\n\```BUG IS RUNNING...```\n\*POWERD BY DIWANBOT*"
...
Asena.addCommand({pattern: 'kbug ?(.*)', fromMe: true, desc: DDO,deleteCommand: true}, (async (message, match) => {    
        if (match[1] == '') {
...
Asena.addCommand({pattern: 'kmybug ?(.*)', fromMe: true, desc: DDO, deleteCommand: true}, (async (message, match) => {    
        if (match[1] == '') {
